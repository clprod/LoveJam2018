...
function EnemyBase:move(dt)
   ...
   if direction.x ~= 0 or direction.y ~= 0 then
	 ...
	 if col.other.name == 'Crystal' then
	    ...
	    -- screenshake
	    --[[
	    local orig_x, orig_y = self.game.camera:position()
	    Timer.during(1, function()
			    self.game.camera:lookAt(orig_x + math.random(-2,2), orig_y + math.random(-2,2))
			    end, function()
			    -- reset camera position
			    self.game.camera:lookAt(orig_x, orig_y)
	    end)
	    ]]--
	 end
